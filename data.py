...
class Data(object):
    ...
    def get_posts_content(self, post_url):
        g = Goose()
        article = g.extract(post_url)
        return {"title": article.title, "content": article.cleaned_text}
    
    def download(self, dst="data/posts"):
        posts_content_table = pd.DataFrame(self.posts_content)
        posts_content_table.to_csv(f'{dst}.csv', index=False, sep="|")
        
        posts_content_table[article] = \
            "==========" + posts_content_table[title] + "==========" + "\n" + posts_content_table["content"]
        full_content = posts_content_table["article"].str.cat(sep="\n")
